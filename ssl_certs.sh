...
generate() {
    ...
    cd sensu_ca
    openssl ecparam -genkey -out private/cakey.pem -name sect283k1
    openssl req -x509 -config openssl.cnf -new -key private/cakey.pem -days 3650 -out cacert.pem -outform PEM -subj /CN=SensuCA/ -nodes
    openssl x509 -in cacert.pem -out cacert.cer -outform DER
    cd ../server
    openssl ecparam -genkey -out key.pem -name sect283k1
